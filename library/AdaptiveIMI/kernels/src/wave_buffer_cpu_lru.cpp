...
private:
    ...
    inline void lru_remove(int64_t key) noexcept {
    ...
    inline void lru_push_front(int64_t key) noexcept {
        auto& desc = cluster_descriptors[key];
        desc.lru_prev = -1;
        desc.lru_next = lru_head;
        if (lru_head >= 0) {
            cluster_descriptors[lru_head].lru_prev = key;
        }
        lru_head = key;
        if (lru_tail < 0) {
            lru_tail = key;
    ...
    inline void removeLeastRecentlyUsed() noexcept {
        ...
        const int64_t lru_key = lru_tail;
        lru_remove(lru_key);

        int* block_ids = cluster_descriptors[lru_key].GPUBlockIDs;
        free_block_ids.insert(free_block_ids.end(), block_ids, block_ids + cluster_descriptors[lru_key].BlockNum);
...
public:
    ...
    WaveBufferCPU_LRU(int batch_size, int group_num, int dim, int nprobe, int new_nprobe, int block_size, 
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    py::class_<WaveBufferCPU_LRU>(m, "WaveBufferCPU_LRU")
        ...
        .def("set_indices", &WaveBufferCPU_LRU::set_indices, 
        ...
        .def("set_kv", &WaveBufferCPU_LRU::set_kv, 
            py::arg("ivf_key"), py::arg("ivf_value"), py::arg("input_keys"), py::arg("input_values"))
        .def("set_cluster_metadata", &WaveBufferCPU_LRU::set_cluster_metadata,
            py::arg("cluster_sizes"), py::arg("cluster_offsets"), py::arg("start_bdx"))
        .def("async_construction", &WaveBufferCPU_LRU::async_construction, 
            py::arg("clusters"), py::arg("cluster_size"), py::arg("start_bdx"))
        .def("construction_sync", &WaveBufferCPU_LRU::construction_sync)
        .def("update_kv", &WaveBufferCPU_LRU::update_kv, 
            py::arg("update_keys"), py::arg("update_values"), py::arg("clusters"), py::arg("cluster_size"), py::arg("searched_clusters"))
        .def("batch_access", &WaveBufferCPU_LRU::para_batch_access, py::call_guard<py::gil_scoped_release>())
        .def("get_last_block_stats", &WaveBufferCPU_LRU::get_last_block_stats)
        .def("sync", &WaveBufferCPU_LRU::sync, py::call_guard<py::gil_scoped_release>());
    
    py::class_<MyThreadPool>(m, "MyThreadPoolLRU", py::module_local())
        .def(py::init<>());
    
    py::class_<ThreadPoolLRU>(m, "ThreadPoolLRU")
        .def(py::init<int>())
        .def("get", &ThreadPoolLRU::get, py::return_value_policy::reference);
}
