...
class ThreadPoolLRUPlus {
    private:
        MyThreadPool* threadpool;
    
    public:
        ThreadPoolLRUPlus(int num_threads) {
...
private:
    ...
    static constexpr double kDefaultHotRatio = 0.3;

    inline int resolve_hot_capacity() const noexcept {
        const char* env_value = std::getenv("IMI_LRU_PLUS_HOT_RATIO");
        double ratio = kDefaultHotRatio;
        if (env_value != nullptr && env_value[0] != '\0') {
            char* endptr = nullptr;
            double value = std::strtod(env_value, &endptr);
            if (endptr != env_value && value > 0.0) {
                ratio = value;
            }
        }
        if (ratio > 1.0) {
        ...
        int target = static_cast<int>(capacity * ratio);
        if (target <= 0) {
            target = std::min(capacity, 1);
        }
        return target;
    }

    inline void list_remove(int64_t key, int64_t& head, int64_t& tail) noexcept {
        auto& desc = cluster_descriptors[key];
        int64_t prev = desc.lru_prev;
        int64_t next = desc.lru_next;
        if (prev >= 0) {
            cluster_descriptors[prev].lru_next = next;
        } else {
            head = next;
        }
        if (next >= 0) {
            cluster_descriptors[next].lru_prev = prev;
        } else {
            tail = prev;
        }
        desc.lru_prev = -1;
    ...
    inline void list_push_front(int64_t key, int64_t& head, int64_t& tail) noexcept {
        auto& desc = cluster_descriptors[key];
        desc.lru_prev = -1;
        desc.lru_next = head;
        if (head >= 0) {
            cluster_descriptors[head].lru_prev = key;
        }
        head = key;
        if (tail < 0) {
            tail = key;
    ...
    inline void demote_hot_if_needed() noexcept {
        while (hot_blocks > hot_capacity && hot_tail >= 0) {
            int64_t demote_key = hot_tail;
            auto& desc = cluster_descriptors[demote_key];
            list_remove(demote_key, hot_head, hot_tail);
            desc.in_hot = false;
            hot_blocks -= desc.BlockNum;
            list_push_front(demote_key, cold_head, cold_tail);
            cold_blocks += desc.BlockNum;
        }
    }

    inline void touch_hot(int64_t key) noexcept {
        if (hot_head != key) {
            list_remove(key, hot_head, hot_tail);
            list_push_front(key, hot_head, hot_tail);
        }
    }

    inline void touch_cold(int64_t key) noexcept {
        if (cold_head != key) {
            list_remove(key, cold_head, cold_tail);
            list_push_front(key, cold_head, cold_tail);
        }
    }

    inline void promote_to_hot(int64_t key) noexcept {
        auto& desc = cluster_descriptors[key];
        if (desc.BlockNum > hot_capacity) {
            touch_cold(key);
            return;
        }
        list_remove(key, cold_head, cold_tail);
        ...
        list_push_front(key, hot_head, hot_tail);
        hot_blocks += desc.BlockNum;
        demote_hot_if_needed();
    }

    inline void removeLeastRecentlyUsed() noexcept {
        int64_t victim = cold_tail >= 0 ? cold_tail : hot_tail;
        if (victim < 0) {
            return;
        }

        auto& desc = cluster_descriptors[victim];
        if (desc.in_hot) {
            list_remove(victim, hot_head, hot_tail);
            hot_blocks -= desc.BlockNum;
        } else {
            list_remove(victim, cold_head, cold_tail);
            cold_blocks -= desc.BlockNum;
        }
        desc.in_hot = false;

        int* block_ids = desc.GPUBlockIDs;
        free_block_ids.insert(free_block_ids.end(), block_ids, block_ids + desc.BlockNum);
...
public:
    BufferManager(
        ...
        int nprobe,
        int block_size,
        int max_consider_block,
        ClusterDescriptor* cluster_descriptors
    ) : capacity(capacity),
        block_size(block_size),
        max_consider_block(max_consider_block),
        cluster_descriptors(cluster_descriptors) {
        hot_capacity = resolve_hot_capacity();
    ...
    inline std::tuple<int, int> batch_update(
        ...
        for (int i = 0; i < admiss_num; ++i) {
            ...
            cluster_descriptor.in_hot = false;
...
public:
    ...
    WaveBufferCPU_LRUPlus(int batch_size, int group_num, int dim, int nprobe, int new_nprobe, int block_size, 
    ...
    void construct_func(
        ...
        for (int idx = _start; idx < _end; ++idx) {
            ...
            for (int i = 0; i < n_centroids; ++i) {
                ...
                cluster_descriptors_group[i].in_hot = false;
                cluster_descriptors_group[i].lru_prev = -1;
                cluster_descriptors_group[i].lru_next = -1;
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    py::class_<WaveBufferCPU_LRUPlus>(m, "WaveBufferCPU_LRUPlus")
        ...
        .def("set_indices", &WaveBufferCPU_LRUPlus::set_indices, 
        ...
        .def("set_kv", &WaveBufferCPU_LRUPlus::set_kv, 
            py::arg("ivf_key"), py::arg("ivf_value"), py::arg("input_keys"), py::arg("input_values"))
        .def("set_cluster_metadata", &WaveBufferCPU_LRUPlus::set_cluster_metadata,
            py::arg("cluster_sizes"), py::arg("cluster_offsets"), py::arg("start_bdx"))
        .def("async_construction", &WaveBufferCPU_LRUPlus::async_construction, 
            py::arg("clusters"), py::arg("cluster_size"), py::arg("start_bdx"))
        .def("construction_sync", &WaveBufferCPU_LRUPlus::construction_sync)
        .def("update_kv", &WaveBufferCPU_LRUPlus::update_kv, 
            py::arg("update_keys"), py::arg("update_values"), py::arg("clusters"), py::arg("cluster_size"), py::arg("searched_clusters"))
        .def("batch_access", &WaveBufferCPU_LRUPlus::para_batch_access, py::call_guard<py::gil_scoped_release>())
        .def("get_last_block_stats", &WaveBufferCPU_LRUPlus::get_last_block_stats)
        .def("sync", &WaveBufferCPU_LRUPlus::sync, py::call_guard<py::gil_scoped_release>());
    
    py::class_<MyThreadPool>(m, "MyThreadPoolLRUPlus", py::module_local())
        .def(py::init<>());
    
    py::class_<ThreadPoolLRUPlus>(m, "ThreadPoolLRUPlus")
        .def(py::init<int>())
        .def("get", &ThreadPoolLRUPlus::get, py::return_value_policy::reference);
}
